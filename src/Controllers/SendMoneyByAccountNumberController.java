...
public class SendMoneyByAccountNumberController {
    @FXML
    ...
    public ObservableList<Consumer> getConsumer(String accNumber) {
//        ObservableList<Consumer> cons = FXCollections.observableArrayList();
//        ArrayList<Consumer> list = manager.getConsumerByAccountNumber(accNumber);
//        for (Consumer c : list){
//            cons.add(c);
//
...
//        }
        ...
        try {
            ...
            RequestAndReply requestAndReply = new RequestAndReply("GET_CONSUMERS_BY_ACC_NUMBER", accNumber);
...
//        ArrayList<Consumer> sender = manager.getConsumerByLogin(login);
        ...
        findConsumerBtn.setOnAction(actionEvent -> {
            ...
            RequestAndReply requestAndReply3 = new RequestAndReply("IS_EXIST_CONSUMER_BY_ACCNUM", accNum);
            try {
                oos.writeObject(requestAndReply3);
                RequestAndReply requestAndReply4 = (RequestAndReply) ois.readObject();
                if(requestAndReply4.isExist()){
                    tableView.setItems(getConsumer(accNum));
                }
                else {
                    JOptionPane.showMessageDialog(null, "There is no such a man with this number", "Incorrect account number!", JOptionPane.ERROR_MESSAGE);
                    accNumField.setText("");
...
//            manager.isExistConsumerByAccNumber(accNum)
        ...
        sendMoneyBtn.setOnAction(actionEvent -> {
            String accNum = accNumField.getText();
            try {
                RequestAndReply request = new RequestAndReply("IS_EXIST_CONSUMER_BY_ACCNUM", accNum);
                oos.writeObject(request);
                RequestAndReply request1 = (RequestAndReply) ois.readObject();
                boolean isExist =request1.isExist();
                if(isExist){
                    RequestAndReply request2 = new RequestAndReply("GET_CONSUMERS_BY_ACC_NUMBER", accNum);
