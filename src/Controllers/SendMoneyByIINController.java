...
public class SendMoneyByIINController {
    @FXML
    ...
    @FXML
    private TextField iinNumField;

    @FXML
    private Button findConsumerBtn;

    public ObservableList<Consumer> getConsumer(String iin) {
//        ObservableList<Consumer> cons = FXCollections.observableArrayList();
//        ArrayList<Consumer> list = manager.getConsumerByIIN(iin);
//        for (Consumer c : list){
//            cons.add(c);
//
...
//        }
        ...
        try {
            ...
            RequestAndReply requestAndReply = new RequestAndReply("GET_CONSUMERS_BY_IIN", iin);
...
//        ArrayList<Consumer> sender = manager.getConsumerByLogin(login);
        ...
        findConsumerBtn.setOnAction(actionEvent -> {
            String iinNum = iinNumField.getText();

            try {
                RequestAndReply requestAndReply3 = new RequestAndReply("IS_EXIST_CONSUMER_BY_IIN", iinNum);
                ...
                if (isExist){
                    tableView.setItems(getConsumer(iinNum));
                }
                else {
                    JOptionPane.showMessageDialog(null, "There is no such a man with this iin number", "Incorrect iin!", JOptionPane.ERROR_MESSAGE);
                    iinNumField.setText("");
        ...
        sendMoneyBtn.setOnAction(actionEvent -> {
            String iinNum = iinNumField.getText();
            try {
                RequestAndReply request = new RequestAndReply("IS_EXIST_CONSUMER_BY_IIN", iinNum);
                oos.writeObject(request);
                RequestAndReply request1 = (RequestAndReply) ois.readObject();
                boolean isExist = request1.isExist();
                if(isExist){
                    RequestAndReply request2 = new RequestAndReply("GET_CONSUMERS_BY_IIN", iinNum);
...
//                        manager.sendMoneyOperation(new Transactions(null, sender.get(0).getId(), receiver.get(0).getId(), money, currDateTime, 0, 0));
                    ...
                    JOptionPane.showMessageDialog(null, "There is no such a man with this number", "Incorrect iin!", JOptionPane.ERROR_MESSAGE);
                    iinNumField.setText("");
                    tableView.setItems(null);
                    moneyAmountField.setText("");
                }
